...
class MyDynamicGraphAdapter(DynamicGraphAdapter):
    ...
    def train_batch_sandwich_with_balanced_flops_sampling(self, inputs, labels=None, **kwargs):
        ...
        for _ in range(self.dyna_bs-2): 
            ...
            config_list = []
            flops_list = []
            for _ in range(3): # sample 3 random path to calculate the flops
...
class Trainer(Model):
    ...
    def train_batch_sandwich(self, inputs, labels=None, **kwargs):
        # call the function train_batch of adapter
        loss = self._adapter.train_batch_sandwich_with_balanced_flops_sampling(inputs, labels, **kwargs)
        if fluid.in_dygraph_mode() and self._input_info is None:
            self._update_inputs()
        return loss
    ...
    def evaluate_whole_test(
        ...
        for arch_name, config in candidate_dict.items():
            ...
            if ParallelEnv().local_rank == 0:
                ...
                with open(f'checkpoints/results/32th_prelu_flops_lam1_sandwich_balance_2times/channel_sample_{num}.txt', 'a') as f:
                    f.write('{}\n'.format(sample_res))
