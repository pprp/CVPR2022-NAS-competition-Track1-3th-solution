...
class OFA(OFABase):
    ...
    def runtime_depth_list(self):
        if self.candidate_depth_list is None:
            return None
        return [random.randint(s, e) for s, e in self.candidate_depth_list]
    ...
    def forward(self, x):
        ...
        # ====================   student process  =====================
        # print(list(self._ofa_layers.keys()))
        if not self._build_ss and self.net_config == None:
            self._clear_search_space(x)
            self._build_ss = True
        ...
        if self.depth_list_manual is None:
            self.depth_list = self.runtime_depth_list()
        else:
            self.depth_list = self.depth_list_manual
                
        if self._broadcast:
            self._broadcast_ss()
        if teacher_output is not None and self.training:
            stu_out = self.model.forward(x, self.depth_list)
            return stu_out, teacher_output
        else:
            return self.model.forward(x, self.depth_list)
