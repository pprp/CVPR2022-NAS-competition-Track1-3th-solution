...
from .utils.utils import compute_start_end, get_same_padding, convert_to_list, make_divisible
    ...
    * :math:`Out`: Output value, the shape of :math:`Out` and :math:`X` may be different.
    ...
    def forward(self, input, kernel_size=None, expand_ratio=None, channel=None):
        ...
        if expand_ratio != None:
            out_nc = make_divisible(int(expand_ratio * self.base_channel), 8)
        elif channel != None:
            out_nc = int(channel)
        else:
...
class SuperBatchNorm2D(nn.BatchNorm2D):
    ...
    def forward(self, input):
        ...
        attrs = ("momentum", self._momentum, "epsilon", self._epsilon, "is_test",
                 ...
                 "fuse_with_relu", False, "use_global_stats", self._use_global_stats,
